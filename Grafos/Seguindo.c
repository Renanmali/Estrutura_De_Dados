...
int contador_seguidos(TVizinho* vizinho){
    if(vizinho == NULL){
        return 0;
    }
    return 1 + contador_seguidos(vizinho->prox);
}
int numero_seguidos(TGrafo *g, char *nome) {
    //TODO: Implementar essa função
    TGrafo* vertice = busca_vertice(g, nome);
    if(g == NULL){
        printf("Não existe esse usuário");
        return 0;
    }
    if(vertice->prim_vizinho != NULL){
        return contador_seguidos(vertice->prim_vizinho);
    }
}
