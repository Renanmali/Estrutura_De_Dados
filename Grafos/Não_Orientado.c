...
int ehNaoOrientado(TGrafo *g) {
    ...
    TVizinho* vizinho;
    while(percorre){
        vizinho = percorre->prim_vizinho;
        while(vizinho){
            TGrafo* percorre_aux = busca_vertice(g, vizinho->id_vizinho);
            TVizinho* vizinho_aux = busca_vizinho(percorre_aux->prim_vizinho, percorre->id_vertice);
            if(vizinho_aux == NULL){
                return 0;
            }
            if(vizinho_aux->peso != vizinho->peso){
                return 0;
            }
            vizinho = vizinho->prox;
        }
